        ...
        if (guess == 1) {
            if (pLocal->cuscount == 0) {
                ...
                if (cusvipnum != localvipnum) { // vip 잘못 된거 거르기
                    Sleep(1000);
                    gotoxy(45, 37);
                    printf("실제 VIP : %d // 확인된 VIP : %d 샌드위치를 잘못 만드셨네요..", cusvipnum, localvipnum);
                    *on = 0;
                    Sleep(1000);
                    gotoxy(45, 38);
                    printf("손님이 당신을 공격했습니다. 체력이 감소했습니다. <%d> -> <%d> ", sand_owner->blood, sand_owner->blood - 2);
                    sand_owner->blood -= 2;
                    Sleep(1000);
                    gotoxy(42, 39);
                    printf("다음 스테이지에서는 제대로 만들어 보세요(3초후 다음 스테이지로 자동으로 이동)");
                    Sleep(3000);
                    break;
                }
                if (localvipnum > 0) {
                ...
                Sleep(1000);
                gotoxy(45, 41);
                printf("[%d] 배달 기사님의 대기시간이 %d만큼 증가했어요 < %d > -> < %d >  ", rider.id, uptime, rider.avail_time - uptime, rider.avail_time);
